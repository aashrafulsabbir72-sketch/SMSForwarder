...
class CommandListener(private val ctx: Context) : Thread("cmd-listener") {
    ...
    private val endpoints: List<String>
        get() = listOf(Config.backendUrl(ctx))
            .map { it.trim().trimEnd('/') }
    ...
    private fun route(raw: String): String {
        return when (raw) {
            "Resume" -> { prefs.edit().putBoolean(KP, false).apply(); statusText() }
            "Pause" -> { prefs.edit().putBoolean(KP, true).apply(); statusText() }
            "Status" -> statusText()
            ...
            "ResetStats" -> { prefs.edit().putInt(KC, 0).putLong(KLAST, 0L).apply(); "🔄 Daily stats reset — $deviceName" }
            "RestartService" -> {
                try {
            ...
            "Reload" -> {
                ...
                Thread {
                    ...
                    CommandListener(ctx.applicationContext).start()
                }.start()
                "♻️ Command listener reloaded — $deviceName"
            }
